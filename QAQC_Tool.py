...
class QAQC:
    ...
    def check_outliers_log_iqr(self, df, dataset_name, factor, columns_to_check=None):
        if df is not None and not df.empty:
            self.report.append("\n" + "=" * 80)
            self.report.append(f"*** Outliers Check ***\n")
            if columns_to_check is None:
                columns_to_check = df.select_dtypes(include=["number"]).columns.tolist() 
            # Attempt to convert non-numeric columns
            ...
            # Final filter for numeric columns
            non_numeric_columns = [col for col in columns_to_check if not np.issubdtype(df[col].dtype, np.number)]
            if non_numeric_columns:
                st.warning(
                    f"Please select only numeric columns for outlier checking. "
                    f"The following columns are non-numeric: {', '.join(non_numeric_columns)}"
                )
                return
            all_outlier_rows = []  # Store all outliers from all columns 
            for column in columns_to_check:
                cleaned_data = df[column].dropna()
                cleaned_data = [x for x in cleaned_data if x > 0]
                if len(cleaned_data) > 0:
                    ...
                    outliers_log_indices = [
                        i for i, x in enumerate(log_transformed_data)
                    ...
                    for value in outliers_original:
                        ...
                        if not matching_indices.empty:
                            outlier_indices.append(matching_indices[0])
                    outlier_indices = list(set([
                        i for i in outlier_indices
                        if i is not None and 0 <= i < len(df)
                    ]))
                    #print("outlier_indicies====\n",outlier_indices)
                    if outlier_indices:
                        outlier_df = df.loc[outlier_indices].copy()
                        #print("outlier_df=====\n", outlier_df)
                        if not outlier_df.empty:
                            #outlier_df["Outlier_Column"] = column
                            #outlier_df["Outlier_Value"] = df.loc[outlier_indices, column]
                            #outlier_df["Row_Index"] = outlier_df.index + 1  # Excel-style indexing
                            self.report.append(f"\nDetected Outlier Rows in '{column}':")   
                            for idx in outlier_df.index:
                                row_number = idx + 1  # excel-style index
                                outlier_val = df.loc[idx, column]
                                self.report.append(f" - Row {row_number} (Index {idx}): {column} = {outlier_val}")
                            st.error(f"Outliers detected in column '{column}'")
                            st.dataframe(outlier_df)
                            all_outlier_rows.append(outlier_df)
                        else:
                            self.report.append(f"No valid outlier rows detected in column '{column}'.")
    ...
    def check_dupes(self, df, cols, dataset_name):
        dupe_cols = cols if cols else df.columns.tolist()
        ...
        if dupes_count > 0:
            duplicate_df = df[df.duplicated(subset=dupe_cols, keep='first')].copy()
            duplicate_indices = duplicate_df.index.tolist()
            excel_indices = [i + 1 for i in duplicate_indices]

            # add Excel-style row index for display
            duplicate_df["Row_Index"] = duplicate_df.index + 1

            # Report
            self.report.append(f"Duplicate record indices (Excel): {excel_indices}")

            # streamlit display
            st.error("Duplicate rows found:")
    ...
    def check_summary(self, df, dataset_name):
        ...
        if whitespace:
            ...
            whitespace_df = pd.DataFrame(
                ...
                columns=["Column Name", "# Of Records with Whitespace"]
            )

            # Display the DataFrame in Streamlit
            st.dataframe(whitespace_df)

            # Also append to the report if needed
            self.report.append("*** Whitespace Check ***\n")
            self.report.append("Whitespace Check detected in the following columns:\n")
            for col, count in whitespace.items():
                self.report.append(f"{col}: {count} rows with leading/trailing whitespace\n")
    ...
    def run_checks(self):
        ...
        if self.dataset1_file:
            ...
            if self.df1 is not None:
                ...
                outlier_bounds = st.select_slider("Multiply the IQR by a factor of:", options=[1.5, 2, 3, 5])               
                self.check_outliers_log_iqr(self.df1, self.dataset1_name, outlier_bounds, selected_columns_df1)
